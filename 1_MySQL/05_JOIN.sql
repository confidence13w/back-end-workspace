...
/*
    ...
    - 연결에 사용하려는 컬럼명이 같은 경우 ON 구문 대친 USING(컬럼명) 구문 사용 
*/
-- 1) 연결할 두 컬럼명이 다른 경우 
...
FROM department
JOIN lacation ON (location_id = local_code);
...
/*
    ...
    - 단, 반드시 기준이 되는 테이블(칼럼)을 지정해야 한다. (LEFT, RIGHT, FULL)
*/
...
SELECT * FROM department; -- dept_id(D*),   location_id(L*)        
SELECT * FROM employee; --   dept_code(D*),                 job_code(J*)
SELECT * FROM job; --                                       job_code(J*)
SELECT * FROM location; --                  local_code(L*),              national_code  
...
SELECT emp_id, emp_name, job_name, sal_level,
	if(sal_level = 'S1' or 'S2', '고급', 
    if(sal_level = 'S3' or 'S4', '중급', '초급'))
FROM employee
	JOIN job USING (job_code)
	JOIN sal_grade ON (min_sal <= salary AND salary <= max_sal);
...
--     참고로 employee, job, departmenr, location, national, sal_grade
